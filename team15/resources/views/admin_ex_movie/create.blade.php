...
<form name = "form1" method="post" action="{{ route('admin_ex_movie.store') }}{{ $tmp }}"
...
<table class="table table-bordered table-sm mymargin5">
    ...
    <td width="20%" class="mycolor2"> 출연 배우</td>
		...
		@foreach ($list1 as $row1)
			@if ( $row1->id == old('actor_id') )
				<option value="{{ $row->id }}" selected>{{ $row1->actor_name }}</option>
			@else
				<option value="{{ $row->id }}">{{ $row1->actor_name }}</option>
			@endif
		@endforeach
            </select>
       </div>

...
<script>
    function find_text()
    {
        form1.action="{{ route('admin_ex_movie.index') }}";
        form1.submit();
    }
</script>
...
<form name="form1" action="" >
    <div class="row">
        ...
        <div class="col-9" align="right">           
             <a href="{{ route('admin_ex_movie.create') }}{{ $tmp }}" class="btn btn-sm mycolor1">추가</a>
        </div>
    </div>
</form>
...
<table class="table  table-bordered  table-sm  table-hover mymargin5">
	...
	<tr>
		<td>{{ $row->id }}</td>
		<td>
			<a href = "{{route('admin_ex_movie.show',$row->id)}} {{ $tmp }}">{{$row-> movie_name}}</a>
		</td>
